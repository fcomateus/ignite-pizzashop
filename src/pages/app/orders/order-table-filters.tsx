...
export function OrderTableFilters() {
    ...
    function handleFilter({ orderId, customerName, status}: OrderFiltersSchema) {
        setSearchParams(urlParams => {
            ...
            if(customerName) {
                urlParams.set('customerName', customerName)
            } else {
                urlParams.delete('customerName', customerName)
            }

            if(status) {
                urlParams.set('status', status)
            } else {
                urlParams.delete('status', status)
            }
    ...
    function handleClerFilters() {
        setSearchParams(urlParams => {
            urlParams.delete('orderId')
            urlParams.delete('customerName')
    ...
    return (
        <form onSubmit={handleSubmit(handleFilter)} className="flex items-center gap-2">
            ...
            <Button onClick={handleClerFilters} type="button" variant='outline' size='xs'>
